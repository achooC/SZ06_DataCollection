...
//			cJSON * id = cJSON_CreateObject();
			...
			if(tb[ZHA_GATEWAY_ATTR_WHITELIST])
			{
				...
				{ 
					...
					{
                        printf("1");
						strncpy(tmp_device.id, blobmsg_get_string(wttb[ZHA_GATEWAY_ATTR_ID]), 21);
...
//						strcpy(device->dsp, dsp);
						sprintf(sql,"INSERT INTO device_tb(date,ieee,ep,did,status) \
							VALUES(date('now'),'%s',%d,%d,%d);",tmp_device.id,tmp_device.ep,tmp_device.did,0);
						printf("sql:[%s]",sql);
						rc = sqlite3_exec(db, sql, NULL, 0, &zErrMsg);
